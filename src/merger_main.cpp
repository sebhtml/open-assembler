	...
	while(reducing){
		...
		for(int i=0;i<contigSequences.size();i++){
			...
			if(sequence.length()<100)
				continue;
			string revWord=(sequence.substr(offset,wordSize));
		...
		for(int i=0;i<contigSequences.size();i++){
			...
			for(int j=0;j<hits.size();j++){
				...
				string endingSeed=QuerySequence.substr(QuerySequence.length()-offset-wordSize,wordSize);
				string startingSeed=(subjectSequenceRaw.substr(offset,wordSize));
...
vector<Read*> reverseOverlapTailToTail(vector<Read*> contigSequences){
	...
	while(reducing){
		...
		for(int i=0;i<contigSequences.size();i++){
			...
			if(sequence.length()<100)
				continue;
			string revWord=DeBruijnAssembler::reverseComplement(sequence.substr(sequence.length()-wordSize-offset,wordSize));
			index[revWord].push_back(i);
		}
		cout<<contigSequences.size()<<" / "<<contigSequences.size()<<endl;
		...
		for(int i=0;i<contigSequences.size();i++){
			...
			for(int j=0;j<hits.size();j++){
				...
				string endingSeed=QuerySequence.substr(QuerySequence.length()-wordSize-offset,wordSize);
				string startingSeed=DeBruijnAssembler::reverseComplement(subjectSequenceRaw.substr(subjectSequenceRaw.length()-offset-wordSize,wordSize));
				int AlignmentStartInQuery=0;
				int AlignmentStartInSubject=0;
				int AlignmentEndInQuery=0;
				int AlignmentEndInSubject=0;
...
int main(int argc,char*argv[]){
	...
	vector<Read*> theBestContigs=reverseOverlapTailToTail(forwardOverlapTailToHead(reverseOverlapHeadToHead(reverseMerge(mergeForward(contigs)))));

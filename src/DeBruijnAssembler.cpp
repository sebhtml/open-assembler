...
void DeBruijnAssembler::outputContigs(ofstream*output){
	...
	for(map<int,list<unsigned long int> >::iterator iteratorContigFirstVertex=m_contig_paths.begin();
		...
		list<unsigned long int>::iterator i=iteratorContigFirstVertex->second.begin();
		unsigned long int currentVertex=*i;
		contigSequence<< idToWord(currentVertex,m_wordSize);
		while(i!=iteratorContigFirstVertex->second.end()){
			//m_cout<<idToWord(currentVertex,m_wordSize)<<endl;
			//m_cout<<currentVertex<<endl;
			currentVertex=*i;
			i++;
			contigSequence<< getLastSymbol(currentVertex);
		}
		string sequenceDNA=contigSequence.str();
